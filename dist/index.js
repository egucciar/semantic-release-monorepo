...
var require_lib3 = __commonJS({
  'node_modules/read-pkg/node_modules/@babel/highlight/lib/index.js'(exports) {
    ...
    function highlightTokens(defs, text) {
      let highlighted = '';
      for (const { type: type3, value } of tokenize(text)) {
  ...
  'node_modules/lru-cache/dist/cjs/index.js'(exports) {
    ...
    var emitWarning = (msg, type3, code, fn) => {
      typeof PROCESS.emitWarning === 'function'
        ? PROCESS.emitWarning(msg, type3, code, fn)
        : console.error(`[${code}] ${type3}: ${msg}`);
    };
    ...
    var LRUCache = class _LRUCache {
      ...
      *#rindexes({ allowStale = this.allowStale } = {}) {
      ...
      #isValidIndex(index) {
        return (
          index !== void 0 && this.#keyMap.get(this.#keyList[index]) === index
        );
      }
      /**
          ...
          if (this.#isStale(index)) {
            ...
            if (!fetching) {
              ...
              if (
                status &&
                allowStale &&
                value.__staleWhileFetching !== void 0
              ) {
  ...
  'node_modules/hosted-git-info/lib/hosts.js'(exports, module) {
    ...
    var defaults = {
      ...
      sshurltemplate: ({ domain, user, project, committish }) =>
        `git+ssh://git@${domain}/${user}/${project}.git${maybeJoin(
          '#',
          committish
        )}`,
      edittemplate: ({
      ...
      browsetreetemplate: ({
        ...
        `https://${domain}/${user}/${project}/${treepath}/${maybeEncode(
          committish || 'HEAD'
        )}/${path8}${maybeJoin('#', hashformat(fragment || ''))}`,
      browseblobtemplate: ({
        domain,
        user,
        project,
      ...
      httpstemplate: ({ auth, domain, user, project, committish }) =>
        `git+https://${maybeJoin(
          auth,
          '@'
        )}${domain}/${user}/${project}.git${maybeJoin('#', committish)}`,
      filetemplate: ({ domain, user, project, committish, path: path8 }) =>
        `https://${domain}/${user}/${project}/raw/${maybeEncode(
          committish || 'HEAD'
        )}/${path8}`,
      shortcuttemplate: ({ type: type3, user, project, committish }) =>
        `${type3}:${user}/${project}${maybeJoin('#', committish)}`,
    ...
    hosts.github = {
      ...
      blobpath: 'blob',
      editpath: 'edit',
      filetemplate: ({ auth, user, project, committish, path: path8 }) =>
        `https://${maybeJoin(
          auth,
          '@'
        )}raw.githubusercontent.com/${user}/${project}/${maybeEncode(
          committish || 'HEAD'
        )}/${path8}`,
      ...
      tarballtemplate: ({ domain, user, project, committish }) =>
        `https://codeload.${domain}/${user}/${project}/tar.gz/${maybeEncode(
          committish || 'HEAD'
        )}`,
      extract: url2 => {
    ...
    hosts.bitbucket = {
      ...
      tarballtemplate: ({ domain, user, project, committish }) =>
        `https://${domain}/${user}/${project}/get/${maybeEncode(
          committish || 'HEAD'
        )}.tar.gz`,
      extract: url2 => {
    ...
    hosts.gist = {
      ...
      browsetemplate: ({ domain, project, committish }) =>
        `https://${domain}/${project}${maybeJoin(
          '/',
          maybeEncode(committish)
        )}`,
      browsetreetemplate: ({
      ...
      filetemplate: ({ user, project, committish, path: path8 }) =>
        `https://gist.githubusercontent.com/${user}/${project}/raw${maybeJoin(
          '/',
          maybeEncode(committish)
        )}/${path8}`,
      shortcuttemplate: ({ type: type3, project, committish }) =>
        `${type3}:${project}${maybeJoin('#', committish)}`,
      ...
      tarballtemplate: ({ project, committish }) =>
        `https://codeload.github.com/gist/${project}/tar.gz/${maybeEncode(
          committish || 'HEAD'
        )}`,
      extract: url2 => {
        let [, user, project, aux] = url2.pathname.split('/', 4);
    ...
    hosts.sourcehut = {
      ...
      blobpath: 'tree',
      filetemplate: ({ domain, user, project, committish, path: path8 }) =>
        `https://${domain}/${user}/${project}/blob/${maybeEncode(committish) ||
          'HEAD'}/${path8}`,
      httpstemplate: ({ domain, user, project, committish }) =>
        `https://${domain}/${user}/${project}.git${maybeJoin('#', committish)}`,
      tarballtemplate: ({ domain, user, project, committish }) =>
        `https://${domain}/${user}/${project}/archive/${maybeEncode(
          committish
        ) || 'HEAD'}.tar.gz`,
          ...
          if (typeof data.bugs === 'string') {
            ...
            if (oldBugs.url) {
              if (
                ...
                url2.parse(oldBugs.url).protocol
              ) {
  ...
  'node_modules/cross-spawn/node_modules/which/which.js'(exports, module) {
    ...
    var which = (cmd, opt, cb) => {
      ...
      const subStep = (p, i, ii) =>
        new Promise((resolve3, reject3) => {
          if (ii === pathExt.length) return resolve3(step(i + 1));
          const ext = pathExt[ii];
          isexe(p + ext, { pathExt: pathExtExe }, (er, is) => {
            if (!er && is) {
              if (opt.all) found.push(p + ext);
              else return resolve3(p + ext);
            }
...
var require_utils = __commonJS({
  'node_modules/braces/lib/utils.js'(exports) {
    ...
    exports.find = (node, type3) =>
      node.nodes.find(node2 => node2.type === type3);
      ...
      const extglobClose = token => {
        ...
        if (token.type === 'negate') {
          ...
          if (
            ...
            const expression = parse2(rest, { ...options, fastpaths: false })
              .output;
    ...
    function readdir(directory2, settings, callback) {
      settings.fs.readdir(directory2, (readdirError, names) => {
        ...
        const tasks = names.map(name => {
          const path8 = common.joinPathSegments(
            directory2,
            name,
            settings.pathSegmentSeparator
          );
...
var require_sync2 = __commonJS({
  'node_modules/@nodelib/fs.scandir/out/providers/sync.js'(exports) {
    ...
    function readdirWithFileTypes(directory2, settings) {
      const dirents = settings.fs.readdirSync(directory2, {
        withFileTypes: true,
      });
...
var require_out3 = __commonJS({
  'node_modules/@nodelib/fs.walk/out/index.js'(exports) {
    ...
    function walk(directory2, optionsOrSettingsOrCallback, callback) {
      if (typeof optionsOrSettingsOrCallback === 'function') {
        new async_1.default(directory2, getSettings()).read(
    ...
    var getGlob = (directory2, options) => {
      ...
      if (options.files && options.extensions) {
        return options.files.map(x =>
          path8.posix.join(directory2, addExtensions(x, options.extensions))
        );
      }
      if (options.files) {
  ...
  'node_modules/ignore/index.js'(exports, module) {
    ...
    var Ignore = class {
      ...
      _t(path8, cache, checkUnignored, slices) {
        ...
        return (cache[path8] = parent.ignored
          ? parent
          : this._testOne(path8, checkUnignored));
      }
      ignores(path8) {
    ...
    var isPathValid = path8 =>
      checkPath(path8 && checkPath.convert(path8), path8, RETURN_FALSE);
    ...
    if (
      ...
      checkPath.isNotRelative = path8 =>
        REGIX_IS_WINDOWS_PATH_ABSOLUTE.test(path8) || isNotRelative(path8);
    }
  },
});
...
var require_gitignore = __commonJS({
  'node_modules/globby/gitignore.js'(exports, module) {
    ...
    var parseGitIgnore = (content, options) => {
      const base = slash(
        path8.relative(options.cwd, path8.dirname(options.fileName))
      );
      return content
        .split(/\r?\n/)
        .filter(Boolean)
    ...
    var getIsIgnoredPredecate = (ignores, cwd) => {
      return p =>
        ignores.ignores(
          slash(path8.relative(cwd, ensureAbsolutePathForCwd(cwd, p.path || p)))
        );
    };
    ...
    module.exports = async options => {
      ...
      const files = await Promise.all(
        paths3.map(file => getFile(file, options.cwd))
      );
...
var require_globby = __commonJS({
  'node_modules/globby/index.js'(exports, module) {
    ...
    module.exports = async (patterns, options) => {
      ...
      const paths3 = await Promise.all(
        tasks.map(task => fastGlob(task.pattern, task.options))
      );
      return arrayUnion(...paths3).filter(
        path_ => !filter2(getPathString(path_))
      );
    };
      ...
      function readdir(path8, options, cb) {
        ...
        var go$readdir = noReaddirOptionVersions.test(process.version)
          ? function go$readdir2(path9, options2, cb2, startTime) {
              return fs$readdir(
                path9,
                fs$readdirCallback(path9, options2, cb2, startTime)
              );
            }
          : function go$readdir2(path9, options2, cb2, startTime) {
              return fs$readdir(
  ...
  'node_modules/is-path-inside/index.js'(exports, module) {
    ...
    module.exports = (childPath, parentPath) => {
      ...
      return Boolean(
        relation &&
          relation !== '..' &&
          !relation.startsWith(`..${path8.sep}`) &&
          relation !== path8.resolve(childPath)
      );
    };
  },
});
  ...
  'node_modules/rimraf/node_modules/glob/common.js'(exports) {
    ...
    function isIgnored(self, path9) {
      ...
      return self.ignore.some(function(item) {
        return (
          item.matcher.match(path9) ||
          !!(item.gmatcher && item.gmatcher.match(path9))
        );
      });
    }
    ...
    GlobSync.prototype._finish = function() {
      ...
      if (this.realpath) {
        ...
        this.matches.forEach(function(matchset, index) {
          var set3 = (self.matches[index] = /* @__PURE__ */ Object.create(
            null
          ));
          for (var p in matchset) {
            try {
              ...
              if (er.syscall === 'stat') set3[self._makeAbs(p)] = true;
              else throw er;
            }
          }
        });
...
var require_glob = __commonJS({
  'node_modules/rimraf/node_modules/glob/glob.js'(exports, module) {
    ...
    glob.hasMagic = function(pattern, options_) {
      ...
      if (set3.length > 1) return true;
      for (var j = 0; j < set3[0].length; j++) {
        if (typeof set3[0][j] !== 'string') return true;
      }
      return false;
    };
    glob.Glob = Glob;
    ...
    Glob.prototype._realpathSet = function(index, cb) {
      ...
      found.forEach(function(p, i) {
        ...
        rp.realpath(p, self.realpathCache, function(er, real) {
          if (!er) set3[real] = true;
          else if (er.syscall === 'stat') set3[p] = true;
    ...
    Glob.prototype._stat = function(f, cb) {
      ...
      if (stat !== void 0) {
        if (stat === false) return cb(null, stat);
          ...
          if (needDir && type3 === 'FILE') return cb();
          else return cb(null, type3, stat);
        }
      }
      var self = this;
...
var require_rimraf = __commonJS({
  'node_modules/rimraf/rimraf.js'(exports, module) {
    ...
    var rmkidsSync = (p, options) => {
      ...
      options
        .readdirSync(p)
        .forEach(f => rimrafSync(path8.join(p, f), options));
...
var require_del = __commonJS({
  'node_modules/del/index.js'(exports, module) {
    ...
    module.exports.sync = (
      patterns,
      { force, dryRun, cwd = process.cwd(), ...options } = {}
    ) => {
      options = {
        expandDirectories: false,
        onlyFiles: false,
...
var require_tempy = __commonJS({
  'node_modules/tempy/index.js'(exports, module) {
    ...
    var writeStream = async (filePath, data) =>
      pipeline(data, fs3.createWriteStream(filePath));
    var createTask = (tempyFunction, { extraArguments = 0 } = {}) => async (
      ...arguments_
    ) => {
  ...
  'node_modules/through2/through2.js'(exports, module) {
    ...
    function through2(construct) {
      return function(options, transform, flush) {
        ...
        if (typeof transform != 'function') transform = noop2;
...
var require_split2 = __commonJS({
  'node_modules/git-log-parser/node_modules/split2/index.js'(exports, module) {
    ...
    function flush(cb) {
      if (this._last) push(this, this.mapper(this._last));
      cb();
    }
...
var require_src2 = __commonJS({
  'node_modules/argv-formatter/src/index.js'(exports) {
    ...
    function args(object) {
      if (object._) {
        return (Array.isArray(object._) ? object._ : [object._]).map(function(
          value
        ) {
    ...
    module.exports = function(child, errFactory) {
      ...
      child.on('close', function(code) {
        if (code !== 0) {
          child.stdout.emit(
            'error',
            errFactory(code, Buffer.concat(stderr).toString())
          );
        }
      });
      return child;
...
var require_src4 = __commonJS({
  'node_modules/git-log-parser/src/index.js'(exports) {
    ...
    function log(args2, options) {
      return fwd(spawn('git', ['log'].concat(args2), options), function(
        code,
        stderr
      ) {
    ...
    exports.parse = function parseLogStream(config2, options) {
      ...
      return combine.obj([
        ...
        through.obj(function(chunk, enc, callback) {
          ...
          callback(
            null,
            map3.reduce(function(parsed, field, index) {
              var value = fields3[index];
              traverse(parsed).set(
                field.path,
                field.type ? new field.type(value) : value
              );
              return parsed;
            }, {})
          );
...
var getErrorLocation = (string, message) => {
  const match = message.match(
    /in JSON at position (?<index>\d+)(?: \(line (?<line>\d+) column (?<column>\d+)\))? while parsing/
  );
  if (!match) {
    return;
  }
  ...
  ({ line, column } = new LinesAndColumns(string).locationForIndex(
    Number(index)
  ));
...
function _isFunction(x) {
  ...
  return (
    type3 === '[object Function]' ||
    type3 === '[object AsyncFunction]' ||
...
function pipeP() {
  ...
  return _arity(
    arguments[0].length,
    reduce_default(_pipeP, arguments[0], tail_default(arguments))
  );
}
...
var filter = /* @__PURE__ */ _curry2(
  /* @__PURE__ */ _dispatchable(['filter'], xfilter_default, function(
    pred,
    filterable
  ) {
...
function _toString(x, seen) {
  ...
  switch (Object.prototype.toString.call(x)) {
    ...
    case '[object Date]':
      return (
        'new Date(' +
        (isNaN(x.valueOf()) ? recur(NaN) : _quote(toISOString_default(x))) +
        ')'
      );
    case '[object Null]':
      return 'null';
...
// node_modules/find-up/index.js
var toPath3 = urlOrPath =>
  urlOrPath instanceof URL ? fileURLToPath3(urlOrPath) : urlOrPath;
...
var makeError = ({
  ...
  killed,
  parsed: {
    options: { timeout, cwd = process4.cwd() },
  },
}) => {
...
var processOk = process8 =>
  !!process8 &&
  typeof process8 === 'object' &&
  typeof process8.removeListener === 'function' &&
  typeof process8.emit === 'function' &&
  typeof process8.reallyExit === 'function' &&
  typeof process8.listeners === 'function' &&
  typeof process8.kill === 'function' &&
  typeof process8.pid === 'number' &&
  typeof process8.on === 'function';
    ...
    for (const sig of signals) {
      this.#sigListeners[sig] = () => {
        ...
        if (listeners.length === count) {
          ...
          if (!ret) process8.kill(process8.pid, s);
        }
      };
    }
...
var {
  ...
  processOk(process5) ? new SignalExit(process5) : new SignalExitFallback()
);
...
var arrayBufferToNodeBuffer = arrayBuffer =>
...
var useTextDecoder = (chunk, { textDecoder }) =>
...
var nativePromisePrototype = /* @__PURE__ */ (async () => {})().constructor
  .prototype;
...
var parseExpression = expression => {
  ...
  if (
    ...
    expression !== null &&
    !(expression instanceof ChildProcess2) &&
...
var normalizeScriptStdin = ({ input, inputFile, stdio }) =>
  input === void 0 && inputFile === void 0 && stdio === void 0
    ? { stdin: 'inherit' }
    : {};
...
// src/git-utils.js
var import_tempy = __toESM(require_tempy(), 1);
var import_git_log_parser = __toESM(require_src4(), 1);
...
var withOptionsTransforms = transforms => plugin => async (
  pluginConfig,
  config2
) => {
...
// src/only-package-commits.js
var memoizedGetCommitFiles = memoizeWith_default(
  identity_default,
  getCommitFiles
);
...
var wrapStep = (
  stepName,
  wrapFn,
  { defaultReturn = void 0, wrapperName = '' } = {}
) => {
  return Array(10)
    .fill(null)
    .map((value, index) => {
...
export { semantic_release_monorepo_default as default };
